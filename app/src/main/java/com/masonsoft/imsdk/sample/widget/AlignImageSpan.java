...
/**
 * copy from https://github.com/wangkunlin/AlignImageSpan
 */
        ...
        if (fm != null) {
            ...
            switch (mVerticalAlignment) {
                case ALIGN_CENTER: { // drawable 的中间与 行中间对齐
                    ...
                    // 算出 ascent 和 descent
                    float ascent = center - drHeight / 2f;
                    float descent = center + drHeight / 2f;
    ...
    public void draw(Canvas canvas, CharSequence text, int start, int end, float x, int top, int y, int bottom, Paint paint) {
        ...
        switch (mVerticalAlignment) {
            case ALIGN_BASELINE:
                ...
                break;
            case ALIGN_CENTER:
                transY = ((bottom - top) - rect.height()) / 2f + top;
                break;
            case ALIGN_TOP:
                transY = top;
                break;
