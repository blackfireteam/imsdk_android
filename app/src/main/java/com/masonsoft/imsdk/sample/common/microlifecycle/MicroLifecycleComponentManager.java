...
public abstract class MicroLifecycleComponentManager implements Closeable, LifecycleEventObserver {
    ...
    protected void dispatchLifecycleEvent(@NonNull Lifecycle.Event event) {
        if (DEBUG) {
            SampleLog.v("dispatchLifecycleEvent %s", event);
        }
    ...
    public void onStateChanged(@NonNull LifecycleOwner source, @NonNull Lifecycle.Event event) {
        if (DEBUG) {
            SampleLog.v("onStateChanged %s", event);
        }
    ...
    public static abstract class MicroLifecycleComponent implements LifecycleOwner {
        ...
        public void onCreate() {
            if (DEBUG) {
                SampleLog.v("onCreate @%s", hashCode());
            }
        }
        ...
        public void onStart() {
            if (DEBUG) {
                SampleLog.v("onStart @%s", hashCode());
            }
        }
        ...
        public void onResume() {
            if (DEBUG) {
                SampleLog.v("onResume @%s", hashCode());
            }
        }
        ...
        public void onPause() {
            if (DEBUG) {
                SampleLog.v("onPause @%s", hashCode());
            }
        }
        ...
        public void onDestroy() {
            if (DEBUG) {
                SampleLog.v("onDestroy @%s", hashCode());
            }
        ...
        private class MicroLifecycleComponentObserver implements LifecycleEventObserver {
            ...
            @Override
            public void onStateChanged(@NonNull LifecycleOwner source, @NonNull Lifecycle.Event event) {
                SampleLog.v("MicroLifecycleComponentObserver onStateChanged %s", event);
                if (event == Lifecycle.Event.ON_CREATE) {
                    ...
                    SampleLog.w("unexpected lifecycle event %s", event);
                }
            }
    ...
    public static class MicroLifecycleComponentRegistry extends LifecycleRegistry {
        ...
        public void handleLifecycleEvent(@NonNull Event event) {
            ...
            if (microLifecycleComponent == null) {
                if (DEBUG) {
                    SampleLog.e("microLifecycleComponent is null");
                }
                return;
            }
