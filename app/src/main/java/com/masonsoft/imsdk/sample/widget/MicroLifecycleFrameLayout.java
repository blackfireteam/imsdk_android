...
public abstract class MicroLifecycleFrameLayout extends FrameLayout {
    ...
    protected static final boolean DEBUG = Constants.DEBUG_WIDGET;
    ...
    protected void toggleWithManual() {
        if (mPerformResumed) {
            ...
            SampleLog.e("ignore toggleWithManual. perform resumed is false.");
        }
    }
    ...
    public void performCreate() {
        if (mCreated) {
            SampleLog.e("ignore, already created");
            return;
        }
    ...
    public void performStart() {
        if (mStarted) {
            SampleLog.e("ignore, already started");
            return;
        }
        mStarted = true;
        onStart();
    ...
    public void performResume() {
        ...
        if (mResumed) {
            SampleLog.v("ignore, already resume");
            return;
        }

        if (mManual && !mAllowResumedOnce) {
            SampleLog.v("can not resume, manual:%s, allow resumed once: %s", mManual, mAllowResumedOnce);
            return;
        }
    ...
    public void performStop() {
        if (!mStarted) {
            SampleLog.e("ignore, not started.");
            return;
        }
        mStarted = false;
        onStop();
    ...
    public void performDestroy() {
        if (!mCreated) {
            SampleLog.e("ignore, not created.");
            return;
        }
        mCreated = false;
        onDestroy();
    ...
    protected void onCreate() {
        if (DEBUG) {
            SampleLog.v(Objects.defaultObjectTag(this) + " onCreate");
        }
    }
    ...
    protected void onStart() {
        if (DEBUG) {
            SampleLog.v(Objects.defaultObjectTag(this) + " onStart");
        }
    }
    ...
    protected void onResume() {
        if (DEBUG) {
            SampleLog.v(Objects.defaultObjectTag(this) + " onResume");
        }
    }
    ...
    protected void onPause() {
        if (DEBUG) {
            SampleLog.v(Objects.defaultObjectTag(this) + " onPause");
        }
    }
    ...
    protected void onStop() {
        if (DEBUG) {
            SampleLog.v(Objects.defaultObjectTag(this) + " onStop");
        }
    }
    ...
    protected void onDestroy() {
        if (DEBUG) {
            SampleLog.v(Objects.defaultObjectTag(this) + " onDestroy");
        }
    }
