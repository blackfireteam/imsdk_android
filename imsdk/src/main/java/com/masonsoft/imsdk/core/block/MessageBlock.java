...
public class MessageBlock {
    ...
    /**
     * 将目标 Message 的 block id 尽可能向外扩展
     *
     * @param sessionUserId
     * @param conversationType
     * @param targetUserId
    ...
    public static void expandBlockId(final long sessionUserId,
        ...
        synchronized (DatabaseSessionWriteLock.getInstance().getSessionWriteLock(databaseHelper)) {
            ...
            if (blockId > 0) {
                ...
                {
                    final long nextBlockId = getMessageBlockId(sessionUserId, conversationType, targetUserId, remoteMessageId + 1);
                    if (nextBlockId > 0 && nextBlockId != blockId) {
                        // 将 nextBlockId 修改为 blockId
                        MessageDatabaseProvider.getInstance().updateBlockId(
                                sessionUserId,
                                conversationType,
                                targetUserId,
