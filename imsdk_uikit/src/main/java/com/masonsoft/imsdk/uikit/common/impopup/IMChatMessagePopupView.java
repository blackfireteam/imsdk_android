...
/**
 * 长按一个消息的弹框视图, 此 View 需要添加在与所在 Window 同样大小的空间上才能显示正确.
 */
public class IMChatMessagePopupView extends ViewGroup {
    ...
    public IMChatMessagePopupView(Context context, AttributeSet attrs) {
    ...
    public IMChatMessagePopupView(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    private int mMenuAnchorSpace = DimenUtil.dp2px(10); // 菜单与 AnchorView 之间的间距
    ...
    private void dispatchMenuClick(int menuId, String menuText, View menuView) {
        if (DEBUG) {
            IMUIKitLog.v("dispatchMenuClick menuId:%s, menuText:%s, menuView:%s",
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        // 在顶部和底部的空间中选一个最大的空间用来显示 menu
        final int topSpace = mAnchorViewRect.top;
        final int bottomSpace = b - mAnchorViewRect.bottom;

        boolean showOnTopOfAnchorView;
        int menuTop;
        if (topSpace >= bottomSpace) {
            // 显示在顶部
            showOnTopOfAnchorView = true;
            menuTop = mAnchorViewRect.top - mMenuAnchorSpace - menuHeight;
        } else {
            // 显示在底部
            showOnTopOfAnchorView = false;
            menuTop = mAnchorViewRect.bottom + mMenuAnchorSpace;
        }
        int menuLeft;
        final int anchorCenterX = mAnchorViewRect.left + (mAnchorViewRect.width() / 2);
        menuLeft = anchorCenterX - menuHalfWidth;
        if (menuWidth < r) {
            // 菜单在水平线上能够完全展示
            if (menuLeft < 0) {
                menuLeft = 0;
            ...
            if (menuLeft + menuWidth > r) {
                // 右边超界
                menuLeft = r - menuWidth;
            }
        }
        ...
        int anchorSpaceDrawableTop;
        if (showOnTopOfAnchorView) {
            anchorSpaceDrawableTop = menuTop + menuHeight;
        } else {
            anchorSpaceDrawableTop = menuTop - anchorSpaceDrawableHeight;
        }
        ...
        mAnchorSpaceDrawable.setReverse(!showOnTopOfAnchorView);
    }
